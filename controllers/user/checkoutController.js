...
const placeOrder = async (req, res) => {
  try {
    ...
    const selectedAddressData = userAddressDoc.address.id(selectedAddress);
    ...
    for (const item of cart.items) {
      ...
      if (!sizeInfo) {
        return res
          ...
          .json({
            success: false,
            error: `Size ${item.size} not found for product: ${product.productName}`,
          });
      }
      ...
      if (sizeInfo.quantity < item.quantity) {
        return res
          ...
          .json({
            success: false,
            error: `Insufficient stock for size ${item.size} of product: ${product.productName}`,
          });
      }
